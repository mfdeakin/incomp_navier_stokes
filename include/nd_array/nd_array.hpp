...
class [[nodiscard]] _nd_array {
 public:
  ...
  constexpr _nd_array(const _nd_array<A_Type, Dims_CT_Array> &src) noexcept {
    for(int i = 0; i < DIMS::product(); i++) {
      vals[i] = src.vals[i];
    }
  }
  ...
  [[nodiscard]] constexpr const A_Type &value(int_t... indices) const noexcept {
  ...
  template <typename... int_t>
  [[nodiscard]] constexpr A_Type &value(int_t... indices) noexcept {
    static_assert(sizeof...(int_t) == DIMS::len(),
                  "Number of indices passed is incorrect");
    return vals[DIMS::slice_idx(indices...)];
  }
  ...
  template <typename... int_t>
  [[nodiscard]] constexpr A_Type &operator()(int_t... indices) noexcept {
    return value(indices...);
  }

  template <typename... int_t>
  [[nodiscard]] constexpr const A_Type &operator()(int_t... indices)
      const noexcept {
    return value(indices...);
  }
  ...
  [[nodiscard]] constexpr _nd_array<
      A_Type,
      typename forward_truncate_array<sizeof...(int_t), Dims_CT_Array>::type>
      &outer_slice(int_t... indices) const noexcept {
    using truncated_dims =
        typename forward_truncate_array<sizeof...(int_t), DIMS>::type;
    using ret_type = _nd_array<A_Type, truncated_dims>;
    return *(reinterpret_cast<ret_type *const>(&vals[0] +
                                               DIMS::slice_idx(indices...)));
  }

  template <typename... int_t>
  [[nodiscard]] _nd_array<A_Type, typename forward_truncate_array<
    ...
    using ret_type = _nd_array<A_Type, truncated_dims>;
    return *(
        reinterpret_cast<ret_type *>(&vals[0] + DIMS::slice_idx(indices...)));
  }

  template <typename Reshaped_Array>
  [[nodiscard]] Reshaped_Array &reshape() noexcept {
    static_assert(Reshaped_Array::DIMS::product() == DIMS::product(),
                  "Reshaped array is not the same size");
    return *reinterpret_cast<Reshaped_Array *>(this);
  }

  // STL compliance
  [[nodiscard]] constexpr A_Type *data() const noexcept { return vals; }
  ...
  class iterator {
   public:
    ...
    constexpr iterator &operator=(const iterator &src) noexcept {
      _val   = src.val;
      _index = src._index;
      return *this;
    }

    constexpr const iterator &operator=(const iterator &src) const noexcept {
      _val   = src.val;
      _index = src._index;
      return *this;
    }
    ...
    [[nodiscard]] constexpr int index(const int dim) const {
      ...
      if(dim + 1 == DIMS::len()) {
        ...
        return (_index / DIMS::trailing_product(dim + 1)) % DIMS::value(dim);
      }
    }

    [[nodiscard]] constexpr reference operator*() const { return *_val; }

    [[nodiscard]] constexpr bool operator!=(const iterator &cmp) const {
    ...
    constexpr iterator(A_Type *pos) : _val(pos), _index(0) {}
  ...
  [[nodiscard]] static constexpr int dimension() { return DIMS::len(); }
...
using ND_Array =
    ND_Array_internals::_nd_array<A_Type,
